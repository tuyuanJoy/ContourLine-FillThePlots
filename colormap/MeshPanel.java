...
public class MeshPanel extends JPanel {
    ...
    private void paintContourLine(Graphics g){
        ...
        //for(int i=0; i<50; i++){
            ...
            //for every triangle, connect to the middle of their edge(if 0-1 or 1-0) to draw a line
            drawContourLine(i, g);
        }
    }
    ...
    private void fillShape(int indexOfColorMap, Graphics g){
       ...
       boolean v1, v2, v3;
        for(int i=0; i<faces.size(); i++){
            ...
            //g.setColor(colors.get(indexOfColorMap));
            pl.addPoint(x[0], y[0]); pl.addPoint(x[1], y[1]); pl.addPoint(x[2], y[2]);
    ...
    private void LocatePanel(Point2D minPoint,Point2D maxPoint){
     ...
     private void scaleToFitWindow(Point2D minPoint, Point2D maxPoint, int width, int height ){
        ...
        double diagonal  = Point2D.distance(minPoint.getX(), minPoint.getY(), maxPoint.getX(), maxPoint.getY());
        double xdistance = maxPoint.getX() - minPoint.getX();
        double ydistance = maxPoint.getY() - minPoint.getY();
        double scaleX = xdistance/diagonal;
        double scaleY = ydistance/diagonal;
        for(int i =0; i< vex.size(); i++){
            ...
            //centerizing 
            x = (x - scaleX * width/2) + width/2;
            y = (y - scaleY * height/2) + height/2;
